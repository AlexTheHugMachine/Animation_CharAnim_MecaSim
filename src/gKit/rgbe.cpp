...
static int rgbe_error( const int rgbe_error_code, const char *msg )
{
...
rgbe2float( float *red, float *green, float *blue, unsigned char rgbe[4] )
{
    ...
    {
        f = std::ldexp( 1.0, rgbe[3] - ( int )( 128 + 8 ) );
        *red = rgbe[0] * f;
        *green = rgbe[1] * f;
        *blue = rgbe[2] * f;
    }
...
int RGBE_WriteHeader( FILE *fp, const int width, const int height, const rgbe_header_info *info )
{
...
int RGBE_ReadHeader( FILE *fp, int *width, int *height, rgbe_header_info *info )
{
    ...
    for ( ;; )
    {
        ...
        else if ( strncmp( buf, "FORMAT=32-bit_rle_rgbe", strlen( "FORMAT=32-bit_rle_rgbe" ) ) == 0 )
            found_format = 1;
        // break;
...
int RGBE_WritePixels( FILE *fp, const float *data, const int n )
{
...
int RGBE_ReadPixels( FILE *fp, float *data, const int n )
{
    unsigned char rgbe[4];
    int numpixels= n;
...
static int RGBE_WriteBytes_RLE( FILE *fp, const unsigned char *data, const int numbytes )
{
...
int RGBE_WritePixels_RLE( FILE *fp, const float *data, const int width, const int n )
{
    unsigned char rgbe[4];
    unsigned char *buffer;
    int i, err;
    
    int scanline_width= width;
    int num_scanlines= n;
...
int RGBE_ReadPixels_RLE( FILE *fp, float *data, const int width, const int n )
{
            ...
            while ( ptr < ptr_end )
            {
                ...
                else
                {
                    ...
                    *ptr++ = buf[1];
                    if ( --count > 0 )
                    {
                        if ( fread( ptr, sizeof( unsigned char )*count, 1, fp ) < 1 )
                        {
                            free( scanline_buffer );
                            return rgbe_error( rgbe_read_error, NULL );
                        }
